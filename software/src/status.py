...
_polling_interval = config.config.get('intervals.polling', 5)
_alert_interval = config.config.get('intervals.alert', 3600)
_last_alert_time = time.time()
_connected = False

_ignored_notifications = 0
_realised_notifications = 0
...
def get_realised_notifications():
    return _realised_notifications
...
def standing():
    ...
    if not _respected_counted_flag:
        ...
        if _sitting_time >= _alert_interval:
            global _realised_notifications
            _realised_notifications += 1
            wm.save_data(_ignored_notifications, _realised_notifications)
